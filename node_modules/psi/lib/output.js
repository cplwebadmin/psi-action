...
module.exports = (parameters, response) => {
  return Promise.resolve().then(() => {
    ...
    const fs = require('fs');
    const cleaned = humanizeUrl(id) + ' - ' + parameters.strategy + ' - ' + convertToPercentum(lighthouseResult.categories.performance.score);//rendered.replaceAll('--------------------------------------------------------', '');
    console.log(cleaned);
    fs.writeFile("/tmp/test", cleaned, function(err) {
        if(err) {
